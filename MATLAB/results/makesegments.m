% [A,SEGCHR,SEGPOS] = MAKESEGMENTS(CHR,POS,M,N) creates contiguous genomic
% segments for genetic loci, in which each segment contains the same number
% of SNPs, M. Inputs CHR and POS are vectors of length P; these vectors
% specify the chromosome number and position along the chromosome (in bases)
% for each SNP. It is assumed that the SNPs are ordered by chromosome
% number, then by position along the chromosome.
%
...
function [A, segchr, segpos] = makesegments (chr, pos, m, n)
