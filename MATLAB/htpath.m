...
% and pathways in the WTCCC study for hypertension (HT).
clear
...
fprintf('INTEGRATED ANALYSIS OF SNPs AND PATHWAYS IN HYPERTENSION.\n');
...
switch stage
 case 'A'
  ...
  save('/tmp/pcarbo/pathway-ht-A.mat','seed','sd','theta0',...
 ...
 case 'B'
  ...
  % importance weight.
  a       = load('/tmp/pcarbo/pathway-ht-A.mat');
  [ans i] = max(a.logw);
  alpha0  = a.alpha(:,i);
  mu0     = a.mu(:,i);
  eta0    = a.eta(:,i);
  ...
  % SAVE RESULTS.
  fprintf('Saving results.\n');
  save('/tmp/pcarbo/pathway-ht-B.mat','seed','sd','theta0',...
       'logw','alpha','mu','s','eta','-v7.3');
 ...
 case 'C'
  ...
  null = getnullstats('/tmp/pcarbo/pathway-ht-A.mat',theta0);
  ...
  % SAVE RESULTS.
  fprintf('Saving results.\n');
  save('/tmp/pcarbo/pathway-ht-C.mat','seed','sd','da','A','H',...
 ...
 case 'D'
  ...
  % SPECIFY PATHWAY HYPOTHESES.
  % I select the pathways with Bayes factors greater than 2 based on the
  % initial calculations in Stage C.
  paths = [  125  396 1065 1087 1095 1171 1172 1187 1441 1838 ...
            1840 1869 2037 2493 2494 2651 2873 3248 3265 3294 ];
  H     = genhmatrix(num2cell(paths),length(pathway.label));

  % Load the posterior statistics under the null hypothesis (Stage B).
  null = getnullstats('/tmp/pcarbo/pathway-ht-B.mat',theta0);
  ...
  % SAVE RESULTS.
  fprintf('Saving results.\n');
  save('/tmp/pcarbo/pathway-ht-D.mat','seed','sd','da','A','H',...
       'theta0','theta','BF','logw1','-v7.3');
end