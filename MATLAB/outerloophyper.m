...
function [logw, alpha, mu, s] = outerloophyper (X, y, alpha, mu, ...
                                                log10sigma, h, log10odds)
  ...
  for i = 1:ns
    fprintf('(%03d) sigma = %0.2e, h = %0.3f, log10odds = %+0.2f ',...
	    i,sigma(i),h(i),log10odds(i));
    fprintf('(sd = %0.3f)\n',sqrt(sa(i)));
  
    % Compute the unnormalized log-importance weight given values for the
    % hyperparameters, LOG10SIGMA, H and LOG10ODDS. Implicitly, the
    % importance weight includes these terms: the likelihood, the prior, and
    % the proposal. The proposal and prior cancel out from the expression
    % for the importance weight because both are assumed to be uniform for
    % all the hyperparameters.
    options = struct('alpha',alpha(:,i),'mu',mu(:,i));
    [logw(i) alpha(:,i) mu(:,i) s(:,i)] = ...
	varbvs(X,y,sigma(i),sa(i),log(10)*log10odds(i),options);
    fprintf('\n');
  end
